...
def safe_fname2camId(x: str, default_cam_id: str = "00000") -> str:
    """
    Compatible camera-id parser.

    Original expected patterns:
        - background: image_00001.jpg -> 00001
        - multiview frame: cam_00001_000123.jpg -> 00001

    New fallback:
        - single-video frame: 000123.jpg -> 00000
    """
    stem = Path(x).stem
    parts = stem.split("_")
    if len(parts) >= 2 and parts[1] != "":
        return parts[1]
    return default_cam_id
...
def background_matting_v2(
    ...
    dataset = ImageFolderDataset(
        ...
        background_fname2camId=lambda x: safe_fname2camId(x, default_cam_id="00000"),
        image_fname2camId=lambda x: safe_fname2camId(x, default_cam_id="00000"),
    )
    dataloader = DataLoader(dataset, batch_size=1, shuffle=False, num_workers=1)
...
def downsample_frames(image_dir: Path, n_downsample: int):
    ...
    for i, image_path in tqdm(enumerate(image_paths), total=len(image_paths)):
        # downsample the resolution of images
